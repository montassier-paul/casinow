    ...
    async (payload: propsData, { dispatch }) => {
        ...
        switch (payload.context) {
            ...
            case ContextTypes.events:
                ...
                dispatch(getEvents())
                try {
                    const data = await axios.get('https://casinow.herokuapp.com/api/' + payload.url, { params: payload.params }).then((res: { data: { data: any } }) =>
                        res.data.data
                    )
            ...
            case ContextTypes.researchBar:
                ...
                try {
                    ...
                    const data = await axios.get('https://casinow.herokuapp.com/api/' + payload.url).then((res: { data: { data: any } }) =>
                        res.data.data
                    )
                    ...
                    switch (payload.userApiParams?.context) {
                        ...
                        case "follow":
                            ...
                            if (payload.userApiParams?.remote && payload.userApiParams.purpose === "jackpot") {
                                ...
                                const remoteUser = await axios.get("https://casinow.herokuapp.com/api/users/user/" + payload.userApiParams?.numTel).then((res: { data: { data: any } }) =>
                                    res.data.data).catch((error) => null)

                                if (remoteUser) {

                                    const res = await axios.put("https://casinow.herokuapp.com/api/users/followings/" + payload.userApiParams?.numTel, { "following": payload.userApiParams.casinoId }).then((res: { data: { data: any } }) =>
                                        res.data.data).catch((error) => null)
                        ...
                        case "delete":
                            ...
                            if (payload.userApiParams?.remote) {
                                ...
                                if (remoteUser) {
                                    const res = await axios.delete("https://casinow.herokuapp.com/api/users/" + payload.userApiParams.numTel).then((res: { data: { msg: any } }) =>
                        ...
                        case "update":
                            ...
                            if (payload.userApiParams?.remote) {
                                ...
                                if (remoteUser) {
                                    ...
                                    const res = await axios.put("https://casinow.herokuapp.com/api/users/" + payload.userApiParams.numTel, payload.userApiParams.body).then((res: { data: { data: any } }) =>
                                        res.data.data).catch((error) => null)

                                }
                                else {
                                    const res = await axios.post("https://casinow.herokuapp.com/api/users/", payload.userApiParams.body).then((res: { data: { data: any } }) =>
                                        res.data.data).catch((error) => null)


                                    payload.userApiParams.followings?.map(async (following) => {
                                        if (following.purpose === "jackpot") {
                                            const res = await axios.put("https://casinow.herokuapp.com/api/users/followings/" + payload.userApiParams?.numTel, { "following": following.casinoId }).then((res: { data: { msg: any } }) =>
                                                res.data.msg).catch((error) => null)
                                        }
                                    })
