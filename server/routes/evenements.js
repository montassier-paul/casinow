...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        if (casino){
            ...
            res.status(200).json({"msg" : "The evenement has been created", "data" : evenement 
            })
...
router.get("/evenement/:id", async (req, res) => {
    ...
    try {
      ...
      const evenement = await Evenement.findById(req.params.id, req.body.projection);
...
router.get("/full/", async (req, res) => {
    ...
    try {
      const evenements = await Evenement.find(req.body.query, req.body.projection).skip(req.body.offset).limit(req.body.limit); 
